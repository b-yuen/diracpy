.. Diracpy documentation master file, created by
   sphinx-quickstart on Mon May 20 09:46:29 2024.
   You can adapt this file completely to your liking, but it should at least
   contain the root `toctree` directive.

Welcome to Diracpy's documentation!
===================================

.. Diracpy, offers a robust API empowering developers to
.. effortlessly craft and resolve quantum models with precision, using a
.. syntax rooted in Dirac notation. Within this API lie pivotal classes for
.. articulating quantum states and operators, along with pre-defined quantum
.. systems. Moreover, it furnishes functions to seamlessly transform and solve
.. these systems.

.. Diracpy transcends mere API functionality. It boasts an extensive
.. collection of methods, designed to tackle quantum dynamics with
.. efficiency and versatility. The true strength of diracpy lies in
.. its ability to liberate developers from the complexities of implementation.
.. By enabling quantum models to be articulated in a succinct, abstract manner
.. using state vectors and operators, diracpy deftly handles the intricate
...
Diracpy's functionality is split between four modules:
...
`states_operators` module
~~~~~~~~~~~~~~~~~~~~~~~~~

The `states_operators` module provides the basic definitions for `bra`, `ket` and `qop` (quantum operator) objects, together with all the necessary binary composition rules and relations needed to define the Hilbert space of `ket` vectors, the dual space of `bra` vectors which maps `ket` vectors to scalars, and the `qop` operators that act on these.

`subspaces` module
~~~~~~~~~~~~~~~~~~

The `subspaces` module contains several pre-defined state spaces, consisting of a rule for how `ket` and `bra` vectors are indexed, and the rules for how the action of the relevant `qop` operators modify these indexes and any complex coefficient when mapping onto the output `bra` or `ket`.

`quantum_systems` module
~~~~~~~~~~~~~~~~~~~~~~~~

The `quantum_systems` module builds a full quantum system from the user-defined Hamiltonian `qop`, and a list of initial states. It uses the Hamiltonian to find states connected to the initial states via the interactions of the Hamiltonian. The construction of the state space is automatically truncated after a user-specified number of interactions. When the number of interactions specified is sufficiently large, the entire closed state space is constructed. The `quantum_system` objects allow states and operators to be automatically vectorized for subsequent numerical simulations of the quantum dynamics.

`quantum_dynamics` module
~~~~~~~~~~~~~~~~~~~~~~~~~

The `quantum_dynamics` module solves the quantum dynamics of the quantum system once it is built. A wide variety of methods are available, allowing one to choose the most efficient method for the particular problem. This includes most prevalent dynamical equations and methods typically found in quantum electrodynamics. This module includes dynamical methods based on numerical integration, methods based on eigen-decomposition, and stochastic methods. It supports both open and closed quantum systems.

Installation
------------

Source code and binary files can be downloaded from `<https://github.com/b-yuen/diracpy>`. To install from the source, navigate to the desired local directory that contains the project. Either download the project into this directory, or use::
...
2. Create a virtual environment using `venv`. In a terminal window navigate to the directory where you want to keep the environment configuration (it does not matter where this is). ::
...
3. Install the dependencies `numpy` and `scipy`: ::
...
.. code-block:: python
    ...
    # Define operator using outer product
    g_projection = ket0 * ket0.conj()
    print(g_projection * ket1)
...
.. code-block:: python
    ...
    # Define initial density matrix and list of times to solve for
...
The next minor version will be released in June 2024 and includes full API documentation and complete integration of the quantum_dynamics module into the front-end. A future major release will address optimization using C.
...
.. toctree::
   ...
   states_operators/index.rst
   subspaces/index.rst
   quantum_systems/index.rst
   quantum_dynamics/index.rst
